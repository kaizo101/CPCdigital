...
describe('regressions', () => {
  ...
  it('does not reopen action after short all-in on later street', () => {
    const players = [
      makePlayer('p1', 100, 0),
      makePlayer('p2', 100, 1),
      makePlayer('p3', 100, 2),
    ...
    const game = new PokerGame(players, { bigBlind: 20, smallBlind: 10, seed: 'short-allin' })
    game.startHand()
    ...
    while (state.phase === 'preflop') {
      ...
      const action = legal?.check ? { type: 'check' as const }
        : legal?.callAmount != null ? { type: 'call' as const }
        : { type: 'fold' as const }
      game.applyAction(state.currentPlayerId, action)
      state = game.getPublicState()
    }

    // Flop: first player checks, second bets
    if (state.currentPlayerId && state.bettingContext?.legalActions.check) {
      game.applyAction(state.currentPlayerId, { type: 'check' })
      state = game.getPublicState()
    }
    if (state.currentPlayerId && state.bettingContext?.legalActions.raise) {
      game.applyAction(state.currentPlayerId, { type: 'raise', amount: 20 })
      state = game.getPublicState()
    }

    // Let remaining players call until p4's turn
    while (state.phase === 'flop' && state.currentPlayerId && state.currentPlayerId !== 'p4') {
      const legal = state.bettingContext?.legalActions
      const action = legal?.callAmount != null ? { type: 'call' as const } : { type: 'fold' as const }
      game.applyAction(state.currentPlayerId, action)
      state = game.getPublicState()
    }

    // Now p4 acts — all-in for 30 (not a full raise)
    if (state.currentPlayerId === 'p4') {
      game.applyAction('p4', { type: 'all-in' })
      state = game.getPublicState()
    }

    // After a non-full-raise all-in that everyone already acted on, betting should end
    if (state.phase === 'flop' && state.currentPlayerId && state.currentPlayerId !== 'p4') {
      // Still players to act — normal continuation
      expect(state.phase).toBe('flop')
    } else {
      expect(state.phase).not.toBe('flop')
    }
  })
})
